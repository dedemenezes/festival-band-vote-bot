const title = 'Rebeca Andrade'
const category = 'esporte'
...
const refreshPage = () => {
  ...
  if (hasObrigado) {
    ...
    closeButton.click()
    // vote again
    voteFor("[ADD NAME HERE]")
  }
}

const vote = () => {
  // const submit = DocumentFragment.querySelector('input.wpcf7-form-control.has-spinner.wpcf7-submit')
  const iframe = document.querySelector(`iframe[src="https://premiobandinspirario.com.br/${category}-${title}/"]`);
...
const voteFor = () => {
  ...
  const festival = Array.from(elements).filter(element => element.textContent.includes(title))[1]
